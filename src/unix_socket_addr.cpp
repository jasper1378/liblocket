...
locket::unix_socket_addr::unix_socket_addr(const std::string& path_or_name, bool is_abstract/*= false*/)
    : m_is_abstract{ is_abstract }
{
    ...
    if (is_abstract)
    {
        std::strncpy((&(m_addr.sun_path[1])), path_or_name.c_str(), (sizeof(m_addr.sun_path) - 2));
    }
    else
    {
        std::strncpy(m_addr.sun_path, path_or_name.c_str(), (sizeof(m_addr.sun_path) - 1));
    }
}
...
socklen_t locket::unix_socket_addr::size() const
{
    return m_k_size;
}
...
sockaddr_un* locket::unix_socket_addr::socket_addr_un_ptr()
{
    return &m_addr;
}
...
bool locket::unix_socket_addr::is_abstract() const
{
    return m_is_abstract;
}

locket::unix_socket_addr& locket::unix_socket_addr::operator=(const unix_socket_addr& other)
{
    if (this == &other)
    {
        return *this;
...
locket::unix_socket_addr& locket::unix_socket_addr::operator=(unix_socket_addr&& other)
{
    if (this == &other)
    {
        return *this;
...
void locket::unix_socket_addr::clear()
{
    std::memset(&m_addr, 0, m_k_size);
}

void locket::unix_socket_addr::set_domain()
{
