...
liblocket::client_stream_socket::client_stream_socket(
...
liblocket::client_stream_socket::client_stream_socket(int sockfd)
...
void liblocket::client_stream_socket::connect(const socket_addr *connect_addr) {
...
std::string liblocket::client_stream_socket::recv(int flags /*= 0*/) const {
  if (m_connected_addr == nullptr) {
...
void liblocket::client_stream_socket::send(const std::string &message,
                                        int flags /*= 0*/) const {
  if (m_connected_addr == nullptr) {
    throw std::runtime_error{"socket is not connected"};
  }
...
liblocket::client_stream_socket &
liblocket::client_stream_socket::operator=(client_stream_socket &&other) noexcept {
  if (this == &other) {
    return *this;
  }
