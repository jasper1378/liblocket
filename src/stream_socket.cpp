...
locket::stream_socket::stream_socket(socket::dummy_type_bind, const socket_addr* bound_addr)
...
locket::stream_socket::stream_socket(int sockfd)
    : socket{ sockfd }
{
...
std::string locket::stream_socket::recv(int flags/*= 0*/) const
{
    ...
    ssize_t bytes_recvd{ ::recv(m_sockfd, message_buffer, m_k_max_message_length, flags) };
    if (bytes_recvd == -1)
    {
...
void locket::stream_socket::send(const std::string& message, int flags/*= 0*/) const
{
