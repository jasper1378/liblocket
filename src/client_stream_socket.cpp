...
locket::client_stream_socket::client_stream_socket(socket_addr::sock_domain domain)
    : stream_socket{ domain },
      m_connected_addr{ nullptr }
{
}

locket::client_stream_socket::client_stream_socket(socket::dummy_type_bind, const socket_addr* bound_addr)
    : stream_socket{ socket::dummy_type_bind{}, bound_addr },
      m_connected_addr{ nullptr }
{
}

locket::client_stream_socket::client_stream_socket(socket::dummy_type_connect, const socket_addr* connected_addr)
    : stream_socket{ connected_addr->domain() },
      m_connected_addr{ nullptr }
{
    connect(connected_addr);
}

locket::client_stream_socket::client_stream_socket(const socket_addr* bound_addr, const socket_addr* connected_addr)
    : stream_socket{ socket::dummy_type_bind{}, bound_addr },
      m_connected_addr{ nullptr }
{
    connect(connected_addr);
...
locket::client_stream_socket::client_stream_socket(client_stream_socket&& other)
...
void locket::client_stream_socket::send(const std::string& message, int flags/*= 0*/) const
{
    if (m_connected_addr == nullptr)
    {
        throw std::runtime_error{ "socket is not connected" };
...
locket::client_stream_socket& locket::client_stream_socket::operator=(client_stream_socket&& other)
{
    if (this == &other)
    {
        return *this;
