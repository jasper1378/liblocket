...
locket::inet4_socket_addr::inet4_socket_addr(in_port_t port)
{
    clear();
    set_domain();
...
locket::inet4_socket_addr::inet4_socket_addr(const std::string& saddr, in_port_t port)
{
    clear();
    set_domain();
    init(saddr, port);
...
locket::inet4_socket_addr::inet4_socket_addr(const sockaddr* other)
{
...
socklen_t locket::inet4_socket_addr::size() const
{
    return m_k_size;
}
...
std::string locket::inet4_socket_addr::to_string() const
{
    ...
    std::string addr_str{};
    addr_str.append(inet_ntop(AF_INET, &m_addr.sin_addr, address_buf, INET_ADDRSTRLEN));
    addr_str.append(":");
...
in_port_t locket::inet4_socket_addr::port() const
{
    return m_addr.sin_port;
}

in_addr locket::inet4_socket_addr::address() const
{
    return m_addr.sin_addr;
}

locket::inet4_socket_addr& locket::inet4_socket_addr::operator=(const inet4_socket_addr& other)
{
    if (this == &other)
    {
        return *this;
...
locket::inet4_socket_addr& locket::inet4_socket_addr::operator=(inet4_socket_addr&& other)
{
    if (this == &other)
    {
        return *this;
...
void locket::inet4_socket_addr::clear()
{
    std::memset(&m_addr, 0, m_k_size);
}

void locket::inet4_socket_addr::set_domain()
{
...
in_addr locket::inet4_socket_addr::resolve_name(const std::string& saddr, byte_order return_order/*= byte_order::net*/)
{
    addrinfo hints;
    ...
    try
    {
        ...
        resolved_address = (reinterpret_cast<sockaddr_in*>(result->ai_addr))->sin_addr;
